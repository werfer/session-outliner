...
function canMove(node, targetNode, position) {
    ...
    if (node instanceof TabNode) {
        ...
        //[ isolate pinned area
        let descendants = model.matchNodes(function(currentNode) {
        ...
        let lastPinnedNode;
        let pinnedArea = descendants.slice();
        let len = descendants.length;
        while (len--) {
            let currentNode = descendants[len];
            if ((currentNode instanceof TabNode) && currentNode.isPinned()) {
                lastPinnedNode = currentNode;
                break;
            }

            pinnedArea.splice(len, 1);
        }
...
function menuCreateNote(model, targetId) {
    let node = TextNode();
    emplaceNode(model, node, targetId);
}
...
const menuEventHandlers = {
    ...
    "createSeparator": menuCreateSeparator,
    "createNote":      menuCreateNote,
