...
// the following function was moved to content.js
/*
...
function onModelTabSelect(event, id) {
    let dispatcher = event.subject;

    dispatcher("tabSelect", id);
}
...
const outliner = function() {
    ...
    let init = function() {
        ...
        model.on("tabSelect",    onModelTabSelect,    dispatchEvent);

//        system.on("quit-application-granted", onQuitApplicationGranted);
    };

    let unload = function(reason) {
        console.log("unload outliner");
