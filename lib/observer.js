...
let InstallHook = function(owner, orgFunc, hookFunc) {
    ...
    // keep weak references to restore the initial state if necessary
    let _owner = components.utils.getWeakReference(owner);
    let _orgFunc = components.utils.getWeakReference(orgFunc);

    let _replacement = function() {
        _hookFunc.apply(owner, arguments);

        if (typeof _orgFunc.get() === "function") {
            _orgFunc.get().apply(owner, arguments);
        }
    };

    _isOwnPropertyDescriptor = Object.getOwnPropertyDescriptor(owner, orgFunc.name);
    if (_isOwnPropertyDescriptor) {
        owner[orgFunc.name] = _replacement;
    } else {
        orgFunc = hookFunc;
        Object.defineProperty(owner, orgFunc.name, {
            writable: true,
            configurable: true,
            value: _replacement
        });
    ...
    let restore = function() {
        if (typeof _owner.get() === "undefined") {
            return;
        }

        let ownerRef = _owner.get();
        let orgFuncRef = _orgFunc.get();

        if (_isOwnPropertyDescriptor) {
            ownerRef[orgFuncRef.name] = orgFuncRef;
        } else {
            // the property belongs to us
            delete ownerRef[orgFuncRef.name];
        }

        // discard hard references
        ownerRef = null;
        orgFuncRef = null;
    }

    let belongsTo = function(obj) {
        return (_owner.get() === obj);
    }
    ...
    let progressListener = {
        onStateChange: function(browser, webProgress, request, flags, status) {
            ...
            if (flags & STATE_START) {
                ...
                // only necessary for STATE_START
...
//                if (hook.owner === window || hook.owner === tabbrowser) {
                if (hook.belongsTo(window) || hook.belongsTo(tabbrowser)) {
                    hook.restore();
                    hooks.splice(len, 1);
                }
            }
